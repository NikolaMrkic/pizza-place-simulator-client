...
var useProps = function useProps(dataSource, columns, onCellChange, form) {
  ...
  var beforeCell = useRef(null); // 使用 useMemo 缓存 editColumns 和 dataIndexMap。 只在 columns 和 curCell 更改后更新

  var _useMemo = useMemo(function () {
    return computedEditColumns(columns, curCell, handleSetCurCell, form);
  }, [columns, curCell]),
  ...
  useTabChange(curCell, handleSetCurCell, cacheSource, dataIndexMap); // 每当 curCell 更变后，更改缓存的 dataSource 。 并且执行 onCellChange
  ...
  useEffect(function () {
    if (beforeCell && beforeCell.current) {
      ...
      var nextSource = produce(cacheSource, function (draft) {
